...
def build_parser():
    p = argparse.ArgumentParser(
        prog="asus2snmp",
        description="Bridge an ASUS router (telnet) to SNMP. Stdlib-only, hand-rolled.",
    )
    p.add_argument("--target", required=True, metavar="HOST[:PORT]",
                   help="ASUS router telnet target (default port 23).")
...
def main(argv=None):
    ...
    log = logging.getLogger("asus2snmp")
    ...
    log.info("ASUS target: telnet %s:%d as %r (poll every %ds)",
